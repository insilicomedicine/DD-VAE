...
def compute_kernel(lls, lsf, x, z):
    ...
    r2 = T.outer(T.sum(x * x / lsre, 1), T.ones_like(z[:, 0: 1])) - \
        np.float32(2) * T.dot(x / lsre, T.transpose(z)) + \
...
def compute_kernel_numpy(lls, lsf, x, z):
    ...
    r2 = np.outer(np.sum(x * x / lsre, 1), np.ones(z.shape[0])) - \
        2 * np.dot(x / lsre, z.T) + np.dot(1.0 / lsre, z.T ** 2)
...
def compute_psi1(lls, lsf, xmean, xvar, z):
    ...
    psi1 = sf * T.outer(constterm2, T.ones_like(z[:, 0: 1])) * T.exp(
        -np.float32(0.5) * r2_psi1)
...
def compute_psi1_numpy(lls, lsf, xmean, xvar, z):
    ...
    r2_psi1 = np.outer(np.sum(xmean * xmean / lspxvar, 1),
                       np.ones(z.shape[0])) - \
        2 * np.dot(xmean / lspxvar, z.T) + \
        np.dot(1.0 / lspxvar, z.T ** 2)
    psi1 = sf * np.outer(constterm2, np.ones(z.shape[0])) * np.exp(
        -0.5 * r2_psi1)
    return psi1
...
def compute_psi2(lls, lsf, z, input_means, input_vars):
    ...
    psi2_computed = sf ** casting(2.0) * \
        term_1[:, None, None] * term_2 * term_3
