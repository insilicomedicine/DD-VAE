...
def global_optimization(grid, lower, upper, function_grid, function_scalar,
    ...
    x_optimal, y_opt, opt_info = spo.fmin_l_bfgs_b(objective, X_initial,
                                                   bounds=lbfgs_bounds,
                                                   iprint=0, maxiter=150)
...
class SparseGP:
    ...
    real numbers in the case of binary classification are 1 or -1 and in
    the case of multiclass classification are 0, 1, 2,.. n_class - 1
    """
    def __init__(self, input_means, input_vars, training_targets,
                 n_inducing_points):

        self.input_means = theano.shared(
            value=input_means.astype(theano.config.floatX), borrow=True,
            name='X')
        self.input_vars = theano.shared(
            value=input_vars.astype(theano.config.floatX), borrow=True,
            name='X')
        ...
        self.sparse_gp = Sparse_GP(n_inducing_points, self.n_points,
                                   self.d_input, self.input_means,
                                   self.input_vars, self.training_targets)
    ...
    # This trains the network via LBFGS as implemented in scipy
    # (slow but good for small datasets)
    ...
    def train_via_LBFGS(self, input_means, input_vars, training_targets,
        ...
        def objective(params):
            ...
            gradient_value = energy_grad(input_means, input_vars,
                                         training_targets)
        ...
        x_opt, y_opt, opt_info = spo.fmin_l_bfgs_b(objective, initial_params,
                                                   bounds=None, iprint=1,
    ...
    def train_via_ADAM(self, input_means, input_vars, training_targets,
        ...
        process_minibatch_adam = theano.function(
            [X, Z, y], -e,
            updates=adam_theano(-e,
                                all_params,
                                learning_rate),
            givens={self.input_means: X,
                    self.input_vars: Z,
        ...
        for j in range(max_iterations):
            suffle = np.random.choice(n_data_points, n_data_points,
                                      replace=False)
            ...
            for i in range(n_batches):
                ...
                process_minibatch_adam(minibatch_data_means,
                                       minibatch_data_vars,
                                       minibatch_targets)
            ...
            for i in range(n_batches):
                ...
                minibatch_data_vars = input_vars[i * minibatch_size: min(
                    (i + 1) * minibatch_size, n_data_points), :]
                pred_new, uncert_new = self.predict(minibatch_data_means,
                                                    minibatch_data_vars)
            ...
            training_ll = np.mean(sps.norm.logpdf(pred - training_targets,
                                                  scale=np.sqrt(uncert)))

            print('Train error: {} Train ll: {}'.format(training_error,
                                                        training_ll))
            sys.stdout.flush()
    ...
    def get_incumbent(self, grid, lower, upper):
        ...
        X = T.matrix('X', dtype=theano.config.floatX)
        function_grid = theano.function([X], m,
                                        givens={self.input_means: X,
                                                self.input_vars: 0 * X})
        function_scalar = theano.function([X], m[0, 0],
                                          givens={self.input_means: X,
                                                  self.input_vars: 0 * X})
        function_scalar_gradient = theano.function(
            [X], T.grad(m[0, 0], self.input_means),
            givens={self.input_means: X,
                    self.input_vars: 0 * X})
    ...
    def optimize_ei(self, grid, lower, upper, incumbent):
        ...
        function_scalar_gradient = theano.function([X],
                                                   -T.grad(log_ei[0, 0], X))

        return \
            global_optimization(grid, lower, upper, function_grid,
                                function_scalar,
    ...
    def batched_greedy_ei(self, q, lower, upper, n_samples=1):
        ...
        randomness_numpy = casting(
            0 * np.random.randn(X_numpy.shape[0], n_samples).astype(
                theano.config.floatX))

        randomness = theano.shared(
            value=randomness_numpy.astype(theano.config.floatX),
            name='randomness', borrow=True)
        X = theano.shared(value=X_numpy.astype(theano.config.floatX), name='X',
                          borrow=True)
        x = T.matrix('x', dtype=theano.config.floatX)
        log_ei = self.sparse_gp.compute_log_averaged_ei(x, X, randomness,
                                                        incumbent)
