...
from .psd_theano import MatrixInversePSD, LogDetPSD
...
class Sparse_GP:
    """
    n_points are the total number of training points
    (that is used for cavity computation)
    """
    ...
    def __init__(self, n_inducing_points, n_points, input_d, input_means,
        ...
        # These are the actual parameters of the posterior distribution
        # being optimzied
        ...
        self.LParamPost = theano.shared(
            value=initial_value.astype(theano.config.floatX),
            name='LParamPost', borrow=True)
        self.mParamPost = theano.shared(
            value=initial_value[:, 0: 1].astype(theano.config.floatX),
            name='mParamPost', borrow=True)
        self.lls = theano.shared(
            value=np.zeros(input_d).astype(theano.config.floatX), name='lls',
            borrow=True)
        self.lsf = theano.shared(
            value=np.zeros(1).astype(theano.config.floatX)[0], name='lsf',
            borrow=True)
        self.z = theano.shared(
            value=np.zeros((n_inducing_points, input_d)).astype(
    ...
    def compute_output(self):
        ...
        self.Kzz = compute_kernel(self.lls, self.lsf, self.z, self.z) + T.eye(
            self.z.shape[0]) * self.jitter * T.exp(self.lsf)
        ...
        self.meanCavity = T.dot(self.covCavity, casting(
            self.n_points - self.set_for_training) / casting(
        ...
        if self.ignore_variances:
            ...
            if exact_output_vars:
                ...
                kk = T.transpose(self.Kxz[:, None, :] * self.Kxz[:, :, None],
                                 [1, 2, 0])
                v1 = T.transpose(T.sum(
                    T.sum(T.shape_padaxis(self.B2, 2) * (self.psi2 - ll), 0),
                    0, keepdims=True))
                v2 = T.transpose(T.sum(
                    T.sum(T.shape_padaxis(self.B, 2) * (self.psi2 - kk), 0), 0,
                    keepdims=True))

            else:

                # We compute the approximate output variance using
    ...
    def getLogNormalizerCavity(self):
        ...
        assert self.covCavity is not None and \
               self.meanCavity is not None and \
               self.covCavityInv is not None

        return (casting(0.5 * self.n_inducing_points * np.log(2 * np.pi)) +
                casting(0.5) * LogDetPSD()(self.covCavity) +
                casting(0.5) * T.dot(T.dot(
                    T.transpose(self.meanCavity),
                    self.covCavityInv),
                self.meanCavity))
    ...
    def getLogNormalizerPrior(self):
        ...
        return casting(
            0.5 * self.n_inducing_points * np.log(2 * np.pi)) - casting(
            0.5) * LogDetPSD()(self.KzzInv)

    def getLogNormalizerPosterior(self):

        assert self.covPosterior is not None and \
               self.meanPosterior is not None and \
               self.covPosteriorInv is not None

        return (casting(0.5 * self.n_inducing_points * np.log(2 * np.pi)) +
                casting(0.5) * LogDetPSD()(self.covPosterior) +
                casting(0.5) * T.dot(T.dot(T.transpose(self.meanPosterior),
                                           self.covPosteriorInv),
                                     self.meanPosterior))

    ##
    # We return the contribution to the energy of the node (See last Eq. of
    # Sec. 4 in http://arxiv.org/pdf/1602.04133.pdf v1)
    #

    def getContributionToEnergy(self):

        assert self.n_points is not None and \
            self.covCavity is not None and \
            self.covPosterior is not None and \
            self.input_means is not None
        ...
        return (((logZcav - logZpost) +
                logZpost / casting(self.n_points) -
                logZprior / casting(self.n_points)) *
                T.cast(self.input_means.shape[0], 'float32') +
                T.sum(self.getLogZ()))
    ...
    def initialize(self):
        ...
        selected_points = np.random.choice(input_means.shape[0],
                                           self.n_inducing_points,
                                           replace=False)
        z = input_means[selected_points, :]

        # If we are not in the first layer, we initialize the length
        # scales to one

        lls = np.zeros(input_means.shape[1])

        M = np.outer(np.sum(input_means ** 2, 1),
                     np.ones(input_means.shape[0]))
        dist = M - 2 * np.dot(input_means, input_means.T) + M.T
        lls = np.log(0.5 * (np.median(
            dist[np.triu_indices(input_means.shape[0], 1)]) + 1e-3)) * np.ones(
            input_means.shape[1])
        ...
        # but with a small random mean so that the outputs are not equal to
        # zero (otherwise the output of the gp will be zero and
        # the next layer will be initialized improperly).

        # If we are not in the first layer, we reduce the variance of
        # the L and m

        L = np.random.normal(
            size=(self.n_inducing_points, self.n_inducing_points)) * 1.0
    ...
    def setForTraining(self):
        ...
        # We only do something if the node was set for prediction
        # instead of training
    ...
    def getLogZ(self):
        ...
        return -0.5 * T.log(2 * np.pi * self.output_vars) - \
               0.5 * (self.training_targets -
                      self.output_means) ** 2 / self.output_vars
    ...
    def compute_log_ei(self, x, incumbent):
        ...
        Kzz = compute_kernel(self.lls, self.lsf, self.z, self.z) + T.eye(
            self.z.shape[0]) * self.jitter * T.exp(self.lsf)
        ...
        v_out = T.exp(self.lsf) + T.dot(Kxz * T.dot(Kxz, B), T.ones_like(
            self.z[:, 0: 1]))  # + T.exp(self.lvar_noise)
        m_out = T.dot(Kxz, KzzInvmeanCavity)
        s = (incumbent - m_out) / T.sqrt(v_out)

        log_ei = T.log(
            (incumbent - m_out) * ratio(s) + T.sqrt(v_out)) + log_n_pdf(s)
    ...
    def compute_log_averaged_ei(self, x, X, randomness, incumbent):
        ...
        new_predictive_mean = T.tile(m_old_x, [1, randomness.shape[1]]) + \
        ...
        log_ei = T.log((incumbent - new_predictive_mean) * ratio(s) + T.sqrt(
            new_predictive_var)) + log_n_pdf(s)
